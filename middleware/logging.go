...
func Logging() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		reg := regexp.MustCompile("(/role|/user|/login|/task)")
		if !reg.MatchString(path) {
			return
		}

		// Skip for the health check requests.
		if path == "/test/health" || path == "/test/ram" || path == "/test/cpu" || path == "/test/disk" {
			return
		}
		...
		if err := json.Unmarshal(blw.body.Bytes(), &response); err != nil {
			log.Errorf(err, "response body can not unmarshal to models.Response struct, body: `%s`", blw.body.Bytes())
